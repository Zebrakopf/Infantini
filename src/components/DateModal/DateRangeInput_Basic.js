...
const DateRangeInput = () =>{
  ...
  const [dateInfo,changeDate] = useState(moment({day:1}))
    ...
    return weekDays.map((day,idx)=>{
      ...
      return(
        <TouchableOpacity key={fieldIdx} style={[styles.field,{marginRight:idx=== 6 ? 0 : 4}]} onPress={()=>{handleDateSelection(fieldDate)}}>
          <View style={[styles.field,{backgroundColor: selected ? '#f4511e' : 'transparent', width:36,height:36,borderRadius:100}]}>
  ...
  const renderDropDown = () => {
    ...
    return (
      <View style={{width:124, height:ITEMHEIGHT * 12 + 8, padding:4, position:"absolute", top:40,left:24,backgroundColor:'#fff' ,zIndex:2,elevation:1}}>
        <Animated.View style={{width:110, height:ITEMHEIGHT, borderRadius:10,backgroundColor:"#f4511e",position:"absolute",zIndex:2,left:5, top:animMonthSelectorPos }}></Animated.View>
        {months.map((month, idx)=>{
          return(
            <TouchableOpacity key={month} style={{width:120,height:ITEMHEIGHT, zIndex:3}} onPress={()=>{changeComponentDate(moment({month:idx,day:1}),idx)}}>
  ...
  return(
    <View style={styles.container}>
      ...
      <View style={styles.bottomButtonLine}>
        ...
        <TouchableOpacity onPress={()=>{}}>
          <Text style={[styles.text,{fontSize:18, color:"#592059", fontWeight:'bold'}]}>CANCEL</Text>
        </TouchableOpacity>
          <TouchableOpacity onPress={()=>{}}>
            <Text style={[styles.text,{fontSize:18, color:"#592059", fontWeight:'bold'}]}>OK</Text>
          </TouchableOpacity>
        </View>
      </View>
    </View>
