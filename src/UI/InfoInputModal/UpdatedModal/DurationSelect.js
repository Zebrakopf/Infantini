...
const DurationSelect = props =>{
  ...
  const onRender = () =>{
    //this function gets called when the durtionimput is first rendered via useEffect. At the moments state cannot persist since children are = null when their infoPage is collapsed....
    console.log("xMinutes", xMinutes, "xHours",xHours)
    let scroll = xHours !== 0 ? xHours + boxHeight*0.3 : 0
    scrollerHoursRef.scrollTo({x:0,y:scroll})
      scroll = xMinutes!== 0 ? xMinutes + boxHeight*0.3 : 0
